<table class="table table-sm mb-0 table-small table-fixed">
    ...
    <tr>
        ...
        <th width="8%"><i title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/syncadmininterface','Last activity ago');?>" class="material-icons">access_time</i></th>
        <th width="15%">
    ...
    <tr ng-repeat="operator in online_op.list track by operator.id">
        ...
        <td title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/syncadmininterface','chats');?>" ng-class="{'align-middle': true, 'text-danger' : (operator.max_chats && operator.max_chats > 0 && operator.max_chats - operator.active_chats <= 0),'text-success' : (operator.max_chats && operator.max_chats > 0 && operator.max_chats - operator.active_chats >= 1)}">
            {{operator.active_chats}} <abbr title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/syncadmininterface','Active chats')?>"><?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/syncadmininterface','a.a')?></abbr>
        </td>

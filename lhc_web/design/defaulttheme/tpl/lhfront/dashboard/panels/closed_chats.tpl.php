          ...
          <div class="panel-list" ng-inf="closed_chats.list.length > 0">
			<table class="table table-sm mb-0 table-small table-fixed list-chat-table">
				...
				<tr ng-repeat="chat in closed_chats.list track by chat.id" ng-click="lhc.startChat(chat.id,chat.nick)" ng-class="{'user-away-row': chat.user_status_front == 2, 'user-online-row': chat.user_status_front == 0}">
					<td>
						<div class="abbr-list" ><a title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/syncadmininterface','Open in a new window');?>" class="material-icons" ng-click="lhc.startChatNewWindow(chat.id,chat.nick,$event)">open_in_new</a><a ng-click="lhc.previewChat(chat.id,$event)" class="material-icons">info_outline</a> {{chat.nick}} </div>
					</td>

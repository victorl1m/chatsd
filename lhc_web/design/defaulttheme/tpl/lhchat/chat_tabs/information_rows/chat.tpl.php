        ...
        <td colspan="2" >
            <h6 class="fw-bold">
                ...
                <?php if ($chat->chat_initiator == erLhcoreClassModelChat::CHAT_INITIATOR_PROACTIVE) : ?>
                    <?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/adminchat','Atendimento proativo')?>
                <?php else : ?>
                    <?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/adminchat','Atendimento')?>
                <?php endif; ?>
                <span class="fs11 ps-1 text-muted fw-normal">
                    <?php if ($chat->status == erLhcoreClassModelChat::STATUS_BOT_CHAT) : ?><?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/adminchat','Automatizado')?>
                ...
                <div class="float-end text-muted">
                    ...
                    <i id="chat-id-<?php echo $chat->id?>-mds" data-chat-status="<?php echo $chat->status?>" data-chat-user="<?php echo $chat->user_id?>" class="material-icons<?php if ($chat->has_unread_op_messages == 1) : ?> chat-unread<<?php endif;?>">brightness_1</i>

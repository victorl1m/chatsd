	...
	if ($chat->user_status != 1) {
		    ...
		    if ($Params['user_parameters_unordered']['eclose'] == 't') {
                ...
                if ($chat->status_sub != erLhcoreClassModelChat::STATUS_SUB_USER_CLOSED_CHAT) {
                    ...
                    $msg->msg = '[level=system-warning exit-visitor]' . ($chat->nick != 'Visitor' ? $chat->nick : htmlspecialchars_decode(erTranslationClassLhTranslation::getInstance()->getTranslation('chat/userleftchat', 'Visitante'), ENT_QUOTES)) . ' ' . htmlspecialchars_decode(erTranslationClassLhTranslation::getInstance()->getTranslation('chat/userleftchat', 'encerrou o atendimento de forma inesperada!') . ' '. erTranslationClassLhTranslation::getInstance()->getTranslation('chat/userleftchat', 'Finalize o atendimento.'), ENT_QUOTES).'[/level]';;
                    $msg->chat_id = $chat->id;
                    $msg->user_id = -1;
                    $msg->time = time();

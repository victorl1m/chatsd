	        ...
	        if ($chat->hash == $hash && $chat->user_status != 1) {
		        	...
		        	$db->beginTransaction();
				        ...
				        if (isset($_GET['eclose']) && $_GET['eclose'] == true) {
				            ...
				            $msg->msg = htmlspecialchars_decode(erTranslationClassLhTranslation::getInstance()->getTranslation('chat/userleftchat','Visitante encerrou o atendimento de forma inesperada!'),ENT_QUOTES);;
				            $msg->chat_id = $chat->id;
				            $msg->user_id = -1;
				            $msg->time = time();
